...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on(
      "state_changed",
      (snapshot) => {
        const uploudProgress =
          (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
      },
      (err) => {
        console.log(err);
        setFields(true);
        setMsg("Error while Uploading : Please Try Agin");
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloudURL) => {
          setImageAsset(downloudURL);
          setIsLoading(false);
          setFields(true);
  ...
  const saveDetails = () => {
    ...
    try {
      if (!title || !calories || !imageAsset || !price || !category) {
        ...
        setIsLoading(false);
        setFields(true);
        setMsg("data Uplouded successfully");
        setAlertStatus("success");
        setTimeout(() => {
          setFields(false);
          clearData();
  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center">
      <div className="w-[90%] md:w-[50%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        ...
        <div
          className="group flex items-center justify-center flex-col border-2 
